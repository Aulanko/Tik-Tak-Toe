...
function Bot_cheater()  {
  ...
  const randomNumGen = (integer)=>{
    const result = Math.floor(Math.random()*(integer))
    return result
  }

  const AImoves = () =>{

    const allBoxes = Array.from(document.querySelectorAll('.cell'))
    const availableBoxes = allBoxes.filter(item => !item.innerText.trim())

    if (availableBoxes.length === 0) return;

    const pick = availableBoxes[randomNumGen(availableBoxes.length)]

    const cellId = parseInt(pick.id)

    setOBoxes([...OBoxes,  cellId])
    pick.innerText = 'O'
    pick.style.color ='white'
    pick.style.backgroundColor = '#34699A'
